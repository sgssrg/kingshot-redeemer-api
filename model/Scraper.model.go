...
type MightPulseScrapeAlliance struct {
	...
	Members            []MPMemberScrape `json:"members"`
...
type MPScrapeAllianceResp struct {
	ErrorState   bool          `json:"error_state,omitempty"`
	Message      string        `json:"message"`
	AllianceInfo *AllianceInfo `json:"alliance_info"`
}

type AllianceInfo struct {
	AllianceName        *string       `json:"alliance_name"`
	AllianceShortCode   *string       `json:"alliance_shortcode"`
	AllianceLeader      *PlayerInfo   `json:"alliance_leader" example:"<-Alliance R5's Info->"`
	AllianceMemberCount *int          `json:"alliance_mem_count" example:"54"`
	LastUpdatedAgo      string        `json:"last_updated" example:"10h ago"`
	AlliancePlayers     *[]PlayerInfo `json:"alliance_players" example:"<-All Member of Alliance->"`
}
type MPAlliancePlayerScaperRouterValidator struct {
	WrongField string `json:"wrong_field" example:"kid || alliance"`
	Message    string `json:"message" example:"Please enter a non-empty string."`
}
